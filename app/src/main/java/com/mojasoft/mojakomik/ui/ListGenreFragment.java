...
import com.mojasoft.mojakomik.adapter.CircleGenreAdapter;
...
public class ListGenreFragment extends Fragment implements View.OnClickListener {

    private ArrayList<ListGenreModel> listGenreModel = new ArrayList<>();
    private CircleGenreAdapter genreAdapter;
    private RecyclerView rvGenre;
    private ProgressDialog progressDialog;
    private TextView txtLainnya;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_list_genre, container, false);

        rvGenre = root.findViewById(R.id.rv_genre_list);
        rvGenre.setHasFixedSize(true);

        txtLainnya = root.findViewById(R.id.txtLainnyaGenre);
        txtLainnya.setOnClickListener(this);
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rvGenre.setLayoutManager(new GridLayoutManager(getActivity(), 4));
        genreAdapter = new CircleGenreAdapter(listGenreModel);
        rvGenre.setAdapter(genreAdapter);

        listGenreModel = new ArrayList<>();

        genreAdapter.renewItems(listGenreModel);

        genreAdapter.setOnItemClickCallback(new CircleGenreAdapter.OnItemClickCallback() {
            @Override
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.txtLainnyaGenre) {
            GridListGenreFragment fragment = new GridListGenreFragment();
            FragmentManager fragmentManager = getParentFragmentManager();

            if (fragmentManager != null) {
                fragmentManager
                        .beginTransaction()
                        .replace(R.id.frameGenre, fragment, GridListGenreFragment.class.getSimpleName())
                        .addToBackStack(null)
                        .commit();
            }
        }
